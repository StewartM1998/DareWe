...
function preload() {
  ...
  // Optional; comment out if file not present:
...
function updateWrappedText() {
  const currentCharsPerLine = getCharsPerLine();
  ...
  wrappedMainText = createCharacterLimitedText((mainText && mainText.length>0)? mainText : def, currentCharsPerLine);
}

function wouldExceedLineLimit(currentText, newChar) {
  const testText = currentText + newChar;
  const currentCharsPerLine = getCharsPerLine();
  const lines = createCharacterLimitedText(testText, currentCharsPerLine);
  return lines.length > getMaxLines();
}

// ---------- Saved posters wall ----------
function loadSavedPosters() {
  const savedPostersData = localStorage.getItem('rgfSavedPosters');
  if (!savedPostersData) return;
  try {
...
function displayPosterInWall(posterData, addToTop=false) {
  ...
  const posterContainer = createDiv().addClass('poster-thumbnail');
  if (posterData.posterSize === 'Landscape') posterContainer.addClass('landscape');
  ...
  posterContainer.mousePressed(() => {
    const index = savedPosters.findIndex(p => p.dataUrl === posterData.dataUrl);
    if (index !== -1) {
...
function createUI(ui) {
  ...
  sizeSelector.changed(() => {
    ...
    if ((posterSize==='Square' || posterSize==='Landscape') && layout===2) {
      layout = 1;
      const ls = select('#layoutSelector'); if (ls) ls.value(1);
    }
    updateLayoutOptions();
  ...
  Object.keys(colors).forEach(col => {
    const buttonContainer = createDiv().addClass('color-button-container');
    ...
    const indicator = createImg(F_UI + 'selected01.png','selected').addClass('selected-indicator');
...
function validateTextLength(currentText = mainTextArea.value()) {
  ...
  if (lines.length > maxLines) {
    let allowedText = '';
    for (let i=0;i<maxLines;i++) { allowedText += lines[i] + (i<maxLines-1?' ':''); }
...
function applyRGFFontToHeaders() {
  ...
  const headings = selectAll('h1, h2, h3, h4, h5, h6');
  for (const h of headings) h.addClass('rgf-font');
...
function initializeSliderPositions() {
  setTimeout(()=>{
    ...
    const sizeImg = select('#sizeSliderContainer img'); if (sizeImg) sizeImg.style('left', (sizePercent*100)+'%');
